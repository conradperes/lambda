...
public class SwingTest extends JPanel implements ActionListener {
	/*
	 * Java 3D applications describe a 3D scene to the rendering system that
	 * produces the rendered frames. The 3D scene description is added to a tree
	 * (directed-acyclic-graph) data structure called a scenegraph. The scenegraph
	 * tree has two major branches: the scene branch describes the objects and
	 * lights in the scene as well as any behaviors that act upon the scene. The
	 * view branch of the scenegraph describes the viewer of the scene, including
	 * their position, screen device, and viewing parameters.
	 *
	 * SwingTest needs to modify the objects on the scene side of the scenegraph at
	 * runtime, so we keep a reference to it in the sceneBranchGroup member variable
	 * below.
	 */
	private BranchGroup sceneBranchGroup = null;

	/*
	 * SwingTest rotates the objects in the scene using a RotationInterpolator
	 * behavior that will automatically modify the rotation components of a 4 × 4
	 * transformation matrix. Objects that are attached to the transformation matrix
	 * object (TransformGroup) will therefore be automatically rotated.
	 */
	private RotationInterpolator rotator = null;

	/*
	 * Java 3D will render the scene into a Canvas3D component. To perform frame
	 * captures and save the frames to disk, we keep a reference to an offscreen
	 * (nonvisible) Canvas3D component that can be explicitly called to render
	 * individual frames.
	 */
	private Canvas3D offScreenCanvas3D = null;

	/*
	 * The image that is attached to the off-screen Canvas3D and contains the
	...
	protected void init() {
		 ...
		 * VirtualUniverse. The VirtualUniverse class contains a number of Locales
		 * objects, each of which describes a discrete region within the scene, and has
		 * its own coordinate system. By deriving your own class from VirtualUniverse
		 * you can define utility methods or additional datastructures for your
		 * application.
		 */
		VirtualUniverse universe = createVirtualUniverse();

		/*
		 * A Locale object allows a coordinate system to be specified for a region
		 * within the scene. By having multiple Locales in your scene you can have
		 * multiple levels of detail without losing coordinate precision due to rounding
		 * errors.
		 */
		Locale locale = createLocale(universe);

		/*
		 * A BranchGroup is a branch of the scenegraph tree. A BranchGroup has a single
		 * parent Node and can have multiple child Nodes. The sceneBranchGroup created
		 * below contains the graphical objects, lights, and behaviors that will compose
		 * the rendered scene.
		 */
		BranchGroup sceneBranchGroup = createSceneBranchGroup();

		/*
		 * A Background Node allows you to specify a colored background, background
		 * image, or background geometry for your application. In this example we simply
		 * create a light-gray background color and add it to the scene side of the
		 * scenegraph. Java 3D will automatically detect that it is a Background Node
		 * and paint the background color into the Canvas3D prior to rendering the scene
		 * geometry.
		 */
		 ...
		 * ViewPlatform. The ViewPlatform defines a location in the scene from which the
		 * scene can be viewed. The scene can contain multiple ViewPlatforms, and View
		 * objects can be moved between them at runtime.
		 */
		ViewPlatform vp = createViewPlatform();

		/*
		 * To contain the ViewPlatform we create a scenegraph branch. We create a
		 * BranchGroup that is the top of the view branch. Underneath it we create a
		 * series of TransformGroup, and then finally we attach the ViewPlatform to the
		 * lowest TransformGroup. The TransformGroups (which contain a 4 × 4
		 * transformation matrix) allow the ViewPlatform to be rotate, scaled, and
		 * translated within the scene.
		 */
		...
		/*
		 * Finally, create the View object and attach it to the ViewPlatform. The View
		 * object has an associated PhysicalEnvironment and PhysicalBody that defines
		 * the characteristics of the viewer and their display hardware. A Canvas3D
		 * rendering component is attached to the View which is used to display the
		 * frames rendered.
		 */
		createView(vp);
	}
	...
	/*
	 * Callback to allow the Canvas3D to be added to a Panel. This method is called
	 * by createView and allows the Canvas3D to be added to its parent GUI
	 * components, in this can as SwingTest is extends JPanel we can just add it
	 * directly to SwingTest.
	 */
	...
	protected View createView(ViewPlatform vp) {
		...
		/*
		 * Set the locations of the clipping planes for the View. Java 3D uses a finite
		 * number of bits (in a depth-buffer) to track the relative distances of objects
		 * from the viewer. These depth-buffer bits are used to track objects between
		 * the front clipping plane and the rear clipping plane. Only objects that fall
		 * between the two clipping planes will be rendered. As the depth-buffer bits
		 * have a finite length (usually 16 or 24 bits) the ratio between the front
		 * clipping plane and the rear clipping plane should be less than about 1000, or
		 * the depth-buffer will be very coarsely quantized and accuracy will be lost.
		 * In this example we use 1.0 for the front clipping plane and 100.0 for the
		 * rear clipping plane.
		 */
	...
	/*
	 * Simple utility method that returns a bounding volume for the application. In
	 * this case we create a spherical volume, centered at 0,0,0 and with a radius
	 * of 100.
	 */
	...
	protected Canvas3D createCanvas3D(boolean offscreen) {
		/*
		 * First we query Java 3D for the available device information. We set up a
		 * GraphicsConfigTemplate3D and specify that we would prefer a device
		 * configuration that supports antialiased output.
		 */
		...
		/*
		 * Here we have hard-coded the initial size of the Canvas3D. However, because we
		 * have used a BorderLayout layout algorithm, this will be automatically resized
		 * to fit—-as the parent JFrame is resized.
		 */
		c3d.setSize(500, 500);
	...
	/*
	 * Get a TransformGroup array for the View side of the scenegraph. We create a
	 * single TransformGroup (which wraps a 4 × 4 transformation matrix) and modify
	 * the transformation matrix to apply a scale to the view of the scene, as well
	 * as move the ViewPlatform back by 20 meters so that we can see the origin
	 * (0,0,0). The objects that we create in the scene will be centered at the
	 * origin, so if we are going to be able to see them, we need to move the
	...
	public TransformGroup[] getViewTransformGroupArray() {
		...
		/*
		 * Here we move the camera BACK a little so that we can see the origin (0,0,0).
		 * Note that we have to invert the matrix as we are moving the viewer not the
		 * scene.
		 */
	...
	protected BranchGroup createSceneBranchGroup() {
		...
		/*
		 * Create a TransformGroup to rotate the objects in the scene and set the
		 * capability bits on the TransformGroup so that it can be modified at runtime
		 * by the rotation behavior.
		 */
		...
		/*
		 * Create an Alpha interpolator to automatically generate modifications to the
		 * rotation component of the transformation matrix. This Alpha loops
		 * indefinitely and generates numbers from 0 to 1 every 4000 milliseconds.
		 */
		Alpha rotationAlpha = new Alpha(-1, Alpha.INCREASING_ENABLE, 0, 0, 4000, 0, 0, 0, 0, 0);

		/*
		 * Create a RotationInterpolator behavior to effect the TransformGroup. Here we
		 * will rotate from 0 to 2p degrees about the Y-axis based on the output of
		 * rotationAlpha.
		 */
		...
		/*
		 * Create the BranchGroup which contains the objects we add/remove to and from
		 * the scenegraph. We store a reference to this subbranch of the scene side of
		 * the scenegraph in a member variable as we need to modify the contents of the
		 * branch at runtime.
		 */
		...
		/*
		 * Wire the scenegraph together. It is useful to do this in the reverse order
		 * that the branches were created— rather like closing parentheses, that way you
		 * will not forget to add a child branch to its parent. If you forget to add a
		 * branch that you have created and populated then it will just not show up in
		 * the scene!
		 */
	...
	/*
	 * Create a BranchGroup that contains a Cube. The User Data for the BranchGroup
	 * is set so the BranchGroup can be identified later. User Data is a field that
	 * you can set on all Nodes in the scenegraph to allow you to associate your own
	 * data with particular scenegraph elements. The Cube must wrapped in a
	 * BranchGroup as only BranchGroups can be added and removed from the scenegraph
	 * at runtime— not Shape3Ds themselves which describe the geometry.
	 */
	...
	/*
	 * AWT callback to indicate that an items has been selected from a menu. This is
	 * not the way to implement menu handling for a large application (!) but it
	 * serves for our simple example.
	 */
	...
	/*
	 * Registers a window listener to handle ALT+F4 window closing. Otherwise the
	 * Swing application will just be made invisible when the parent frame is
	 * closed.
	 */
	 ...
	 * and creates the JPanel which is the application itself.
	 */
	public static void main(String[] args) {
		/*
		 * Tell Swing that we need Popup Menus to be heavyweight. The Java 3D window is
		 * a heavyweight window – that is, the window is a native window, and therefore
		 * any windows that must overlap it must also be native. Our menu items will be
		 * dropped down in front of the Java 3D Canvas3D so they must be created as
		 * heavyweight windows.
		 */
		JPopupMenu.setDefaultLightWeightPopupEnabled(false);

		/*
		 * Similarly we can declare that ToolTip windows are created as heavyweight. Our
		 * application does not use tooltips. However, if a toolbar was added the
		 * tooltips would overlap the Canvas3D and would also need to be heavyweight
		 * windows.
		 */
		...
		/*
		 * Create a JMenuBar that will generate the events for the application. We
		 * register the swingTest instance as a listener for the action events generated
		 * by the menu items.
		 */
